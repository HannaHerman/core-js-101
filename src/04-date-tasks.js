...
function timeSpanToString(startDate, endDate) {
  ...
  const milliseconds = (endDate.getTime() - startDate.getTime()).toString().slice(-3);
...
function angleBetweenClockHands(date) {
  ...
  const hourAngle = 30 * hour + minutes / 2;
  const minAngle = 6 * minutes;
  const diff = hourAngle - minAngle;
  const angle = diff > 360 ? Math.abs(360 - diff) : diff;
